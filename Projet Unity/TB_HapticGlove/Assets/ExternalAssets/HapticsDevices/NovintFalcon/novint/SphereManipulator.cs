...
public class SphereManipulator : MonoBehaviour
{
    ...
    public bool GetFalconIsOccuped()
    {
        return falconOccuped;
    }
    public void SetFalconIsOccuped(bool value)
    {
        falconOccuped = value;
    }

    public void ApplyMassToFalcon(float mass)
    {
        FalconUnity.setSphereGodObject(falcon_num, godObject.localScale.x * godObject.GetComponent<SphereCollider>().radius, godObjectMass + mass, godObject.position, minDistToMaxForce * hapticTipToWorldScale, maxDistToMaxForce * hapticTipToWorldScale);
    }
}
