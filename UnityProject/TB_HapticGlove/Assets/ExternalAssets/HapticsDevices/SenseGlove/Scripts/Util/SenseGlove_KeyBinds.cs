...
public class SenseGlove_KeyBinds : MonoBehaviour
{
    ...
    [Tooltip("The key to enter to enable or disable wrist calibration")]
    public KeyCode enableWristCalibration = KeyCode.O;
    ...
    void Update ()
    {
        ...
        if(Input.GetKeyDown(this.enableWristCalibration))
        {
            SenseGlove_HandModel sL = this.senseGlove.GetComponent<SenseGlove_VirtualHand>();
            if (sL.updateWrist == true)
            {
                Debug.Log("Disable Update Wrist");
                //this.senseGlove.SetUpdateWrist(false);
                sL.updateWrist = false;
            }
            else if(sL.updateWrist == false)
            {
                Debug.Log("Enable Upate Wrist");
                //this.senseGlove.SetUpdateWrist(true);
                //
                sL.updateWrist = true;
                sL.CalibrateWrist();
            }
