...
public class Senso_Material : MonoBehaviour
{
    ...
    {
        if(FindParentWithTag(other.gameObject, "SensoGlove"))
        {
            if (isGrabbable
                && !isObjectPickedUp
                && !FindParentWithTag(other.gameObject, "SensoGlove").GetComponent<SensoHandExample>().GetHandIsOccuped())
                {
    ...
    /// AND the distance between the two fingers is smaller or equals than distPinch, the object is picked up
    /// Set hand is occuped to true, prevents 2 objects from being picked up with the same hand
    /// </summary>
    private void PickupObject()
    {
        if (thumbColliding
            && indexColliding
            && Vector3.Distance(thumb.transform.position, index.transform.position) <= distPinch)
        {
