...
/// </summary>
public class SenseGlove_DeviceManager : MonoBehaviour
{
    //--------------------------------------------------------------------------------------------------------
    // Properties
    ...
    protected static List<SenseGlove> detectedGloves = new List<SenseGlove>();
    ...
    protected virtual void Cleanup()
    {
        ...
        SenseGlove_Object.deviceScannerPresent = false;
    }
    ...
    {
        //Debug.LogError("Started Device Scanning, we should now be able to detect gloves.");
    ...
    public virtual int GlovesDetected
    {
        get { return detectedGloves.Count; }
    }


    public virtual bool GlovesAttached
    {
        get { return myMethodCalled; }
    }
    ...
    protected void Start()
    {
        ...
        SenseGlove[] glovesCurrentlyInMemory = SenseGloveCs.DeviceScanner.SenseGloves;
        if (glovesCurrentlyInMemory.Length > detectedGloves.Count)
        {
            for (int i = 0; i < glovesCurrentlyInMemory.Length; i++)
                this.queuedGloves.Add(glovesCurrentlyInMemory[i]);
        }
    }

    protected virtual void SceneManager_sceneUnloaded(UnityEngine.SceneManagement.Scene arg0)
    {
        this.Cleanup();
        UnityEngine.SceneManagement.SceneManager.sceneUnloaded -= SceneManager_sceneUnloaded;
    }
