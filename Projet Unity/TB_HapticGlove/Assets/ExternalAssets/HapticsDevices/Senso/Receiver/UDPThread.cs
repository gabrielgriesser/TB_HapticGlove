...
namespace Senso
{
    public class UDPThread : NetworkThread
    {
        ...
        ///
        /// @brief Send vibrating command on each finger to the server
        ///
        public override void VibrateEachFinger(EPositionType handType, ushort duration, byte strength)
        {
            if (m_sock != null)
            {
                sendToServer(GetVibrateFingerJSON(handType, EFingerType.Thumb, duration, strength));
                sendToServer(GetVibrateFingerJSON(handType, EFingerType.Index, duration, strength));
                sendToServer(GetVibrateFingerJSON(handType, EFingerType.Middle, duration, strength));
                sendToServer(GetVibrateFingerJSON(handType, EFingerType.Third, duration, strength));
                sendToServer(GetVibrateFingerJSON(handType, EFingerType.Little, duration, strength));
            }
